...
(
	...
	PIF_AMT DECIMAL(15,0), -- 일시불
	INST_AMT DECIMAL(15,0), -- 할부사용금액
	OVRS_AMT DECIMAL(15,0), -- 해외사용금액
	CASH_AMT DECIMAL(15,0) -- 현금서비스
);
...
where party_nm = '임하영';
-- coalesce(num, num2) : num 이 null 이면 num2 리턴

select * from card_tran_201311;

-- 1. cmf, 이름, 총 사용금액을 총 사용금액 기준 내림차순으로 출력하시오
select cmf, party_nm as '이름', 
	   (coalesce(pif_amt, 0) + coalesce(inst_amt, 0) + 
        coalesce(ovrs_amt, 0) + coalesce(cash_amt, 0)) as 총사용금액
...
select cmf, party_nm as '이름', pif_amt*0.01 as '캐시백'
from card_tran_201311
where seg = 'PB';

-- 4. 고객 등급별 총사용 금액을 추출하고 총사용금액 기준 내림차순 정렬
select seg, sum(coalesce(pif_amt, 0) + coalesce(inst_amt, 0) + 
...
-- 5. 사용금액이 가장 많은 서비스는
select sum(coalesce(pif_amt, 0)) as 일시불, sum(coalesce(inst_amt, 0)) as 할부,
	   sum(coalesce(ovrs_amt, 0)) as 해외사용, sum(coalesce(cash_amt, 0)) as 현금서비스
from card_tran_201311;

-- 6. 사용횟수가 가장 많은 서비스는
select count(pif_amt) as 일시불, count(inst_amt) as 할부,
	   count(ovrs_amt) as 해외사용, count(cash_amt) as 현금서비스
from card_tran_201311;
...
id int,
purchase_amt int, -- 올해 구매 금액
purchase_cnt int, -- 올해 구매 건수
last_amt int, -- 작년 구매 금액
last_cnt int); -- 작년 구매 건수
...
-- 3. 올해 구입건수가 작년 구입건수 보다 많은 고객들의 고객번호, 올해구입건수, 작년구입건수를 출력하고 
-- 올래구입건수를 기준으로 오름차순 정렬하시오.
...
select id, round(purchase_amt/purchase_cnt, 2) as 올해_평균_구매금액
from purchase_tran;

-- 5. 올해와 작년의 구매건수 당 평균 구매금액을 구하시오
select id, round(purchase_amt/purchase_cnt, 2) as 올해_평균_구매금액,
		   round(last_amt/last_cnt, 2) as 작년_평균_구매금액
from purchase_tran;